		...
		btnTitle.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (isTitleUsed(titleField)) {
					...
					JOptionPane.showMessageDialog(null, "This title is OK", "OK Title", JOptionPane.PLAIN_MESSAGE);
				}
			}
		});
		btnTitle.setForeground(Colors.ORANGE);
		btnTitle.setFont(new Font("Dialog", Font.BOLD, 12));
		btnTitle.setFocusPainted(false);
		btnTitle.setBackground(new Color(22, 28, 35));
		btnTitle.setBounds(12, 207, 101, 22);
	...
	protected boolean isTitleUsed(JTextField title) {
		for (User user : User.getAllUsers()) {
			...
			for (Content content : user.getConentPersonal()) {
				if (title.getText() == content.getTitle()) {
					return true;
				}
			}
		}
